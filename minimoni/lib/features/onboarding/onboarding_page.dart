...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  ...
  void _nextStep() async {
    if (_currentStep < _totalSteps - 1) {
      ...
      // Onboarding tamamlandı, bebek bilgilerini kaydet
      await _saveBabyAndComplete();
    }
  }

  Future<void> _saveBabyAndComplete() async {
    // Form validasyonu
    if (_babyName.isEmpty || _babyBirthDate == null || _babyGender.isEmpty) {
    ...
    try {
      // Baby model oluştur - ID otomatik generate edilecek
      ...
      // Storage service ile kaydet (yeni addBaby metodu kullan)
      final success = await BabyStorageService.instance.addBaby(baby);
      ...
      if (success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Bilgiler kaydedilemedi, tekrar deneyin'),
              backgroundColor: Colors.red,
            ),
          );
        }
      ...
      print('Bebek bilgisi kaydetme hatası: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
